 ...
 */
export function safeWhichSync(exe: string): string {
...
/**
 * Same as safeWhichSync(), except it uses async I/O operations under the hood.
 */
export async function safeWhich(exe: string): Promise<string> {
  if (canRunDirectly(exe)) {
    return exe;
  }
